    ...
    public function get_maths_questions_post()
    {
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($this->post('type') && $this->post('type_id')) {
                ...
                if (!empty($data)) {
                    // classifing questions type to two different array
                    for ($i = 0; $i < count($data); $i++) {
                        if($data[$i]['question_type'] == 1){
                            //if maths question type is 1 then store data in maths_question_data_options
                            ...
                            //if maths question type is 2 then store data in maths_question_data_true_false
                            $data[$i]['image'] = ($data[$i]['image']) ? base_url() . MATHS_QUESTION_IMG_PATH . $data[$i]['image'] : '';
                            unset($data[$i]['optione']);
                            $data[$i]['optiona'] = trim($data[$i]['optiona']);
                            $data[$i]['optionb'] = trim($data[$i]['optionb']);
                            unset($data[$i]['optionc']);
                            unset($data[$i]['optiond']);
                            $data[$i]['answer'] = $this->encrypt_data($firebase_id, trim($data[$i]['answer']));
                        }
                    }
                    $response['error'] = false;
            ...
            if ($user_id && $this->post('type')) {
                ...
                if ($type == 1 || $type == '1') {
                    ...
                    if (!empty($data)) {
                        ...
                        $response['error'] = true;
                        $response['message'] = "102111";
                    }
                } else {
                    $response['error'] = true;
                    $response['message'] = "102343";
                }
            } else {
                $response['error'] = true;
                $response['message'] = "103";
    ...
    public function get_system_configurations_post()
    {
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            $setting = [
                ...
                'quiz_zone_duration', 'self_challange_max_minutes', 'guess_the_word_seconds', 'maths_quiz_seconds', 'fun_and_learn_time_in_seconds','battle_mode_one','battle_mode_group', 'true_false_mode'
