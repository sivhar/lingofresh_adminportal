...
class Dashboard extends CI_Controller {
    ...
    public function __construct() {
       ...
       $this->result['half_logo'] = $this->db->where('type', 'half_logo')->get('tbl_settings')->row_array();
       $this->NO_IMAGE = base_url() . LOGO_IMG_PATH . (isset($this->result['half_logo']['message']) ? $this->result['half_logo']['message'] : 'default.png');
    }
    ...
    public function index() {
        ...
        $this->result['week_data'] = $this->db->query("SELECT m.name as day_name, (SELECT COUNT(id) AS user_count FROM tbl_users WHERE DAYOFWEEK(date_registered) = DAYOFWEEK('" . $this->toDate . "') AND DAYNAME(date_registered) = m.name GROUP BY DAYOFWEEK(date_registered)) as user_count FROM tbl_month_week m WHERE m.type=2")->result();
        $this->result['day_data'] = $this->db->query("SELECT COUNT(id) AS user_count, DAYOFMONTH(date_registered) AS day_name FROM tbl_users WHERE MONTH(date_registered) = MONTH('" . $this->toDate . "') GROUP BY DATE(date_registered)")->result();
